...
| `scan` | `--file PATH` | 掃描指定的本地檔案，印出包含關鍵字的行數與內容 | `python main.py scan --file "./sample.log"` |
| `monitor` |`--file PATH` <br> `--interval FLOAT` |進入持續監控模式，每隔指定秒數自動掃描檔案內容。|`python main.py monitor --file "./sample.log" --interval 5`|
